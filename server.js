    ...
    database('classes').select()
        ...
        .catch(error => {
            response.status(500).json({
                error: 'Unable to retrieve classes'
            });
        });
});
        ...
        .then(classes => {
            if (classes.length) {
                ...
                response.status(404).json({
                    error: `No class with ID: ${request.params.id} exists`
                });
            };
        });
});
    ...
    database('subclasses').where('id', request.params.id).select()
        .then(subclasses => {
            if (subclasses.length) {
                ...
                response.status(404).json({
                    error: `No subclass with ID: ${request.params.id} exists`
                });
            };
        });
});
...
app.post('/api/v1/subclasses', (request, response) => {
    ...
    for (let requiredParameter of['name', 'subclass_flavor', 'description', 'parent_class']) {
        if (!subclass[requiredParameter]) {
            response.status(422).send({
                error: `Required format is: { name: <string>, subclass_flavor: <string>, description: <string> } ${requiredParameter} missing from request`
            })
        }
    }
    database('classes').where('name', subclass.parent_class).select()
        .then(parentClass => {
            ...
            database('subclasses').insert({
                    name,
                    subclass_flavor,
                    description,
                    class_id: parentClass[0].id
                }, 'id')
                .then(newId => {
                    const {
                        name,
